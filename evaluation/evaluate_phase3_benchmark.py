...
RETRIEVER_MODE = os.getenv("RETRIEVER_MODE", "hybrid").strip().lower()


def _selected_retriever_mode() -> str:
    mode = RETRIEVER_MODE
    if mode in {"colbert", "zero", "zero_shot", "zero-shot", "colbert_zero"}:
        return "colbert_zero"
...
class PipelineColBERTSelector(DocumentSelector):
    """
    ColBERT-backed selector.

    Keeps the same interface as PipelineHybridSelector so the
    benchmark code does not need to change.
    """
...
def build_selector(project_root: Path, docs_dir: str, embeddings_file: str):
    mode = _selected_retriever_mode()

    if mode == "hybrid":
        print("RETRIEVER_MODE: hybrid -> PipelineHybridSelector")
        return PipelineHybridSelector(
            docs_dir=docs_dir,
    ...
    if mode == "colbert_zero":
        ...
        return PipelineColBERTSelector(
            project_root=project_root,
            experiment="phase3_colbert",
            index_name="phase3_colbert_zero",
        )

    if mode == "colbert_ft":
        print("RETRIEVER_MODE: colbert_ft -> PipelineColBERTSelector (phase3_colbert_ft)")
        return PipelineColBERTSelector(
            project_root=project_root,
            experiment="phase3_colbert",
            index_name="phase3_colbert_ft",
